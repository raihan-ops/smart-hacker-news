...
export function useCommentsPagination(storyId: number, depth: number | 'all' = 1, limit = 20) {
  ...
  const currentPage = parseInt(searchParams.get(queryParams.commentPage) || '1', 10);
  const offset = (currentPage - 1) * limit;

  const query = useQuery<CommentsResponse>({
    queryKey: queryKeys.comments.list(storyId, depth, currentPage),
    queryFn: async () => {
  ...
  const setPage = (page: number) => {
    ...
    params.set(queryParams.commentPage, page.toString());
